...
import { AdminAccess, alreadyExisted, invalidUser, successConstant } from './utils/constants/error.constants';
  ...
  describe('TEST LOGIN', () => {
    ...
    it ('should return [SUCCESS] getPrivilegeAdmin', async() => {
      try {
        ...
        expect(dataAdmin).toEqual({...successConstant, userType: UserType.ADMIN})
      } catch (error) {
        console.log(error)
      }
    })
    it ('should return [SUCCESS] getPrivilegeCustomer', async() => {
      try {
        ...
        expect(dataCustomer).toEqual({...successConstant, userType: UserType.CUSTOMER})
      } catch (error) {
        console.log(error)
      }
    })
